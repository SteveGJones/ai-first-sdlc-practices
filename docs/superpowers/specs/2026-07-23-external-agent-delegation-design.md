...
### 9.15 Permission model + machine-framed channels (2026-07-24, authoritative)

Probing the two CLIs' permission behavior and their machine-framed agent channels reshaped the posture model and the Mode B design:

**agy headless permission reality.** In `--print` mode agy **auto-denies every tool permission** — `read_file`, `command`, `write_file`, `edit_file` — unless the tool is pre-listed in the **global** `~/.gemini/antigravity-cli/settings.json` `permissions.allow` (entries like `command(<prefix>)`, `read_file(<target>)`), or `--dangerously-skip-permissions` is passed. **`--mode` (plan/accept-edits) and `--sandbox` do NOT gate tools in headless mode** — they are interactive-mode concepts. There is no project/workspace-level settings merge for permissions (only the global file). Confirmed live: a read task → `read_file` auto-denied; a write task under `--mode accept-edits --sandbox` → still `read_file` auto-denied. Consequence: the original `read-only=plan+sandbox` / `workspace=accept-edits+sandbox` mapping is largely inert for real headless work; the real lever is `permissions.allow` (via an alternate config dir) or `--dangerously-skip-permissions`.

**codex has a structured per-action approval protocol; agy does not.** `codex app-server generate-json-schema` reveals a full JSON-RPC protocol with server→client approval requests: `ExecCommandApprovalRequest` (`{command:[argv], cwd, parsedCmd, reason, callId}`), `ApplyPatchApprovalRequest`, `FileChangeRequestApprovalParams`, `PermissionsRequestApprovalParams`, `McpServerElicitationRequest`, `ToolRequestUserInput` — answered with a `ReviewDecision` ∈ {`approved`, `approved_for_session`, `denied`, `abort`, policy-amendment variants}. This is exactly a per-action, posture-driven approval loop, machine-framed (no ANSI/pty scraping). agy's machine channel is `agentapi` (a shim → `agy agentapi`) exposing only `new-conversation <prompt>` / `send-message <id> <content>` / `get-conversation-metadata` — a conversation RPC with **no approve/deny verb**.

**Design consequences (roadmap):**
- **Mode A (`--print`/`exec resume`)** stays the simple, robust default — but agy Mode A is limited to whatever is pre-allowed; codex Mode A read-only works (its `-s read-only` is a real sandbox, not a per-tool prompt).
- **Mode B for codex = the app-server JSON-RPC session + a posture→ReviewDecision policy** (approve reads/exec by posture, gate `ApplyPatch`/`FileChange` to workspace/dangerous, deny destructive). This is the flagship "we are the permission authority" capability and the home for delegating real write-work safely. Deferred to a dedicated sub-epic (decided 2026-07-24: finish Mode A cleanups first).
- **agy graded postures** come from an alternate config dir via `agy --gemini_dir <dir>` carrying a posture-appropriate `permissions.allow` (read-only list vs read+write list), NOT from `--mode`/`--sandbox`. Viability of `--gemini_dir` + auth-sharing is the gating probe for the DF1 fix. If unavailable, fall back to documenting a one-time global allow-list + honest `NO_OUTPUT` status (DF2).

### 9.16 DF1 + DF2 implementation notes (issue #232, this build)

Both fixes from §9.15's "Design consequences" landed in `extdel.sh`, the mock CLIs, and both wrapper agents' docs:

**DF1 (agy graded postures via `--gemini_dir`).** `submit_agy_turn` now writes/refreshes a per-handle config dir (`./tmp/agent-delegation/<HANDLE>/agy-cfg/antigravity-cli/settings.json`) before every invocation — start turn 1 AND every resume turn, so a `--steal` posture change is picked up automatically — and passes `--gemini_dir "$dir/agy-cfg"` on every call. `read-only` grants `read_file(*)` + read-only command prefixes only; `workspace` additionally grants `write_file(*)`/`edit_file(*)` + a build/test command set; `dangerous` passes `--dangerously-skip-permissions` (a config dir is still written for invocation uniformity, but its content is inert once that flag is present). `--mode`/`--sandbox` are dropped from the agy posture mapping entirely — confirmed inert headless. `preflight_agy`'s initialized-check still reads the REAL `~/.gemini/antigravity-cli/` (auth lives there), per the task's own instruction — the per-handle dir carries only permissions, not credentials.

Id capture (`agy_read_last_conversations_id`/`capture_agy_session_id`/`agy_check_identity_drift`) now reads `last_conversations.json` from that same per-handle isolated cache dir, not a machine-global path. Because each handle's cache is now exclusive to it, the §5.2/§9.6 first-call `mkdir` lock that used to serialize same-cwd id-*reads* across concurrent handles was removed — there is nothing shared left to race over for that specific hazard. (A same-cwd race against the user's own *interactive* agy, which still uses the real `~/.gemini`, remains structurally moot: that agy never writes into our `--gemini_dir` at all.)

**DF2 (`NO_OUTPUT` status).** `cmd_status` no longer reports `SUCCESS` for an exit-0 turn whose captured answer is empty after trimming whitespace — new status `NO_OUTPUT`, added to the §4.2 vocabulary. The downgrade happens *after* agy's turn-1 id capture (still worth doing — the conversation exists server-side even when the specific tool call inside it was denied) but before status_errors is composed. When the turn's stderr/CLI-log matches an auto-deny-shaped pattern (`permission_denial_hint`, checked against `no output produced|auto-denied|required the .*permission|permission.*denied`), that line is surfaced verbatim in `## Errors` alongside concrete next-step guidance; a genuinely empty-but-legitimate answer still reports `NO_OUTPUT`, just without the permission hint. Kept CLI-general per the task: codex gets the same exit-0-but-empty check (an empty `-o` file), the permission-hint match is attempted there too though it's an agy-shaped signal in practice.

Both wrapper agents (`agy-runner.md`, `codex-runner.md`), the `agent-delegation-policy` skill, and the plugin README were updated to document the new posture mechanism and `NO_OUTPUT` semantics; codex's future per-action approval Mode B (§9.15's "Design consequences") is noted as a forward reference only, not attempted here. agy `persistent`/Mode B remains explicitly out of scope and unimplemented (`extdel.sh` still refuses it with `Status: ERROR`).
