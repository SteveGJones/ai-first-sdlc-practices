...
**Unique handles (precondition).** Before the gate, `library_specs` is deduplicated by handle in first-appearance order, with a stderr warning per dropped duplicate (handles from `--libraries`/`resolve_dispatch_list`/`library_specs_override` are NOT guaranteed unique — e.g. `--libraries local` colliding with implicit `local`, or `a,a`). This is required because the RRF-is-interleaving invariant (§2) assumes each `(handle, page_id)` appears in exactly one ranked list; duplicate handles would otherwise reinforce RRF scores and double-audit. A duplicate-handle test asserts the dedup + warning.

The linear function then validates EVERY (deduplicated) resolved library **once**, before searching, and uses the loaded stores directly (no later reload — closes the gate/worker TOCTOU):
...
8. **Read across libraries (path-containment enforced)** — for each chosen qualified id: `handle, page_id = split_qualified(qid)`; resolve `target = (path_for_handle / page_id).resolve()` and **require `target` to be within `path_for_handle.resolve()`** (reject/skip traversal like `../secret.md` — defense-in-depth even though step 3's live-corpus intersection already drops non-corpus ids); read `target`; pages keyed by the qualified id; skip ids whose file is missing.
...
### Spec-review round-5 resolutions

- **[P1] duplicate library handles** → dedupe `library_specs` by handle (first-appearance) with a stderr warning before the gate; preserves the RRF one-list-per-item invariant + prevents double-audit (§3). Duplicate-handle test.
- **[P1] search hits unconstrained to live corpus (path-traversal risk)** → intersect each library's search hits with `{page_id from chunk_pages(path)}` before filter/fusion (drops corrupt/stale/tampered rows incl. `../secret.md`), AND enforce resolved-path containment within the library at read time (§5 step 3 + step 8). Corrupt/nonexistent/traversal row tests.
