...
def _client(**kw) -> MlxClient:
    return MlxClient(model="mlx-community/Qwen2.5-Coder-14B-Instruct-4bit", **kw)


def test_payload_carries_the_full_conversation() -> None:
    messages = [
...
def test_stop_token_is_always_sent() -> None:
    """mlx_lm.server >=0.31.3 leaks <|im_end|> into content without it."""
    assert "<|im_end|>" in _client().build_payload([])["stop"]


def test_max_tokens_defaults_high_enough_for_multi_file_answers() -> None:
...
def test_extract_content_reads_the_reply() -> None:
    data = {"choices": [{"message": {"content": "```file:a.py\nX\n```"}}]}
    assert MlxClient.extract_content(data) == "```file:a.py\nX\n```"


def test_extract_content_tolerates_a_malformed_reply() -> None:
