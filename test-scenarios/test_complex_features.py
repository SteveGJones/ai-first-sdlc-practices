...
This is a simple bug fix that updates a single function.""")
        ...
        has_skip = any("No plans required" in str(r)
...
- Multiple components to be updated""")
        ...
        has_error = any("requires implementation plan" in str(r)
                        for r in validator.results)
        print(
            f"  {'✅' if has_error else '❌'} Complex feature without plan: error detected")
        ...
        print(
            f"  {'✅' if has_success else '❌'} Complex feature with plan: validation passed")
...
def test_retrospective_staleness():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  {'✅' if has_success else '❌'} Fresh retrospective: validation passed")
        ...
        has_warning = any("No retrospective found" in str(r)
                          for r in validator.results)
        print(
            f"  {'✅' if has_warning else '❌'} Missing retrospective: warning detected")
