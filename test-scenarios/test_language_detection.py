...
spec = importlib.util.spec_from_file_location("setup_smart", str(setup_smart_path))
...
def test_empty_repo_detection():
    ...
    for name, files, expected_empty in test_cases:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"{status} {name}: expected {empty_str}, got {'empty' if validator.is_empty_repo else 'not empty'}")
