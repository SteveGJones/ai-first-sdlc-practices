...
""")
        ...
        subprocess.run(["git", "checkout", "-b", "feature/add-api"], cwd=tmpdir, capture_output=True)
...
""")
        ...
        spec = __import__('importlib.util').util.spec_from_file_location("validate_pipeline", str(val_path))
        validate_pipeline = __import__('importlib.util').util.module_from_spec(spec)
        ...
        print(f"  Empty repo detection: {'✅ Correctly detected as non-empty' if not validator.is_empty_repo else '❌ Incorrectly detected as empty'}")
        
        # Test language detection
        setup_path = Path(__file__).parent.parent / "setup-smart.py"
        spec = __import__('importlib.util').util.spec_from_file_location("setup_smart", str(setup_path))
        ...
        print(f"  Language detection: {'✅' if lang == 'python' else '❌'} Detected as '{lang}'")
