...
class ChemistryCalculator:
    ...
    def calculate_collaboration_score(self, pr_data: Dict) -> float:
        ...
        decision_phrases = ['we should', "let's", 'we can', 'our approach', 'team decided']
        for comment in comments:
            text = comment.get('body', '').lower()
    ...
    def calculate_billy_wright_score(self, pr_data: Dict) -> float:
        ...
        setup_phrases = ['this will help', 'this enables', 'this allows', 'making it easier']
        comments = pr_data.get('comments', [])
        ...
        team_first_phrases = ['for the team', 'team needs', 'helps everyone', 'our goal', 'we need']
    ...
    def generate_chemistry_report(self, score: float) -> Dict[str, Any]:
        ...
        report = {
            'score': score,
            'level': level,
            'badge_color': 'gold' if level == 'legendary' else 'green' if level == 'advanced' else 'yellow' if level == 'developing' else 'orange',
...
def main():
    ...
    parser.add_argument('--output', choices=['score', 'json'], default='score',
                       help='Output format')
