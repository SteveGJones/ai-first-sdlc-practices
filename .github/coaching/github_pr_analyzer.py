...
class GitHubPRAnalyzer:
    ...
    def fetch_pr_data(self, pr_number: int) -> PRCollaborationData:
        ...
        for commit in commits_data:
            processed_commits.append({
                ...
                'author': commit.get('author', {}).get('login', '') or 
    ...
    def _analyze_comments(self, comments: List[Dict]) -> Dict[str, Any]:
        ...
        consultation_keywords = ['@', 'please review', 'thoughts?', 'what do you think', 
                                'could you', 'can you check', 'need input']
        consultation_mentions = sum(
            1 for c in comments 
            if any(keyword in c.get('body', '').lower() for keyword in consultation_keywords)
        )
        
        # Look for positive collaborative language
        positive_keywords = ['great', 'excellent', 'thanks', 'good work', 'nice', 
                           'appreciate', 'helpful', 'agree', 'love it']
        positive_language = sum(
            1 for c in comments 
            if any(keyword in c.get('body', '').lower() for keyword in positive_keywords)
        )
    ...
    def _analyze_commits(self, commits: List[Dict]) -> Dict[str, Any]:
        ...
        co_authored = sum(
            1 for c in commits 
            if 'co-authored-by' in c.get('message', '').lower()
        )
        
        # Check for teammate references
        teammate_keywords = ['per', 'as discussed', 'following', 'based on', 
                           'suggested by', 'reviewed by', 'with']
        references_teammates = sum(
            1 for c in commits 
            if any(keyword in c.get('message', '').lower() for keyword in teammate_keywords)
        )
    ...
    def _check_billy_wright_principles(self, pr_data: PRCollaborationData) -> Dict[str, bool]:
        ...
        if any(phrase in all_text_lower for phrase in 
               ['please review', 'need input', 'what do you think', 'thoughts?']):
        ...
        if any(phrase in all_text_lower for phrase in 
               ['this will help', 'this enables', 'makes it easier', 'sets up']):
            principles['setup_teammates'] = True
        
        # Check for celebrating others
        if any(phrase in all_text_lower for phrase in 
               ['great work', 'thanks', 'excellent', 'well done', 'appreciate']):
    ...
    def _calculate_chemistry_factors(self, pr_data: PRCollaborationData, 
    ...
    def _generate_recommendations(self, score: float, analysis: Dict) -> List[str]:
        ...
        if analysis['billy_wright_indicators'].get('team_first_language') == False:
