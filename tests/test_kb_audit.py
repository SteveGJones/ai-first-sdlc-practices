...
def test_read_log_filter_by_event_type(tmp_path: Path) -> None:
    ...
    log_event(
        log_path,
        AuditEvent(
            "2026-04-26T15:30:00Z", "attribution_drop_retrieval", "q", "local", "r", {}
        ),
    )
    log_event(
        log_path,
        AuditEvent(
            "2026-04-26T15:30:01Z", "synthesis_aborted_attribution", "q", None, "r", {}
        ),
    )
    log_event(
        log_path,
        AuditEvent(
            "2026-04-26T15:30:02Z", "attribution_drop_retrieval", "q", "corp", "r", {}
        ),
    )
...
def test_read_log_filter_by_source_handle(tmp_path: Path) -> None:
    ...
    log_event(
        log_path,
        AuditEvent(
            "2026-04-26T15:30:01Z", "attribution_drop_retrieval", "q", "corp", "r", {}
        ),
    )
...
def test_read_log_malformed_lines_skipped(tmp_path: Path) -> None:
    ...
    log_event(
        log_path,
        AuditEvent(
            "2026-04-26T15:30:01Z", "attribution_drop_retrieval", "q", "local", "r", {}
        ),
    )
