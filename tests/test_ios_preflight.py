...
def test_coremotion_without_motion_string_is_error() -> None:
...
def test_short_single_word_string_is_placeholder() -> None:
    plist = {"NSCameraUsageDescription": "Camera"}
    findings = check_usage_descriptions(plist, ["AVFoundation"])
    assert _codes(findings) == {"placeholder-usage-description"}


def test_framework_not_in_map_is_ignored() -> None:
    findings = check_usage_descriptions({}, ["SwiftUI", "Combine"])
    assert findings == []


def test_case_insensitive_framework_match() -> None:
    findings = check_usage_descriptions({}, ["coremotion"])
    assert _codes(findings) == {"missing-usage-description"}


def test_present_but_placeholder_key_without_detected_framework() -> None:
...
def test_missing_manifest_with_required_reason_api_is_error() -> None:
...
def test_missing_manifest_with_sdks_is_error() -> None:
    findings = check_privacy_manifest(
...
def test_missing_manifest_without_triggers_is_info() -> None:
    findings = check_privacy_manifest(
...
def test_present_manifest_passes() -> None:
    findings = check_privacy_manifest(
...
def test_push_production_aps_passes() -> None:
    findings = check_entitlements(
...
def test_detect_used_frameworks_swift_import(tmp_path) -> None:
    _write(
...
def test_detect_third_party_sdks_via_podfile(tmp_path) -> None:
    _write(tmp_path / "Podfile", "pod 'Firebase'\n")
    assert detect_third_party_sdks(tmp_path) is True


def test_detect_third_party_sdks_absent(tmp_path) -> None:
...
def test_run_end_to_end_flags_missing_motion_string(tmp_path) -> None:
...
def test_run_clean_project_has_no_errors(tmp_path) -> None:
