...
- **Recursive dogfood is the highest-yield validation per hour.** 10 findings in 22 tasks (5 IMPORTANT, 5 MINOR after the 2026-05-01 architect review which raised F-007 severity and added F-010). Internal architectural review and unit tests would not have surfaced F-008 (validator-vs-convention mismatch) — only real use did. The 5 IMPORTANT findings are 5 distinct defects, not one root cause masquerading as five (an earlier version of this retrospective claimed they clustered; that claim was wrong — see §"What we deferred" below).
...
- **Findings reinforce but do not cluster.** An earlier version of this retrospective claimed F-001, F-008, F-009 share a single root cause and one v0.2.0 fix resolves all three. The 2026-05-01 architect review challenged that claim; on recheck the reviewer is correct. F-001 is parser coverage, F-008 is traceability semantics, F-009 is export-evidence completeness. The three reinforce each other — fixing the parser without fixing the granularity semantics still produces noisy `granularity_match` warnings; fixing both without typed evidence statuses still produces a misleading RTM. v0.2.0 must address all three plus F-007 (visibility extraction) and F-010 (REQ-quality discipline) coordinated, not as a single-fix shortcut.
...
Items the dogfood (Phase F) surfaced as v0.2.0 priorities (10 findings after the 2026-05-01 architect review):

**The five IMPORTANT findings — three distinct assurance blockers plus two discipline gaps:**

1. **F-001 (IMPORTANT) — parser coverage:** Extend annotation handling beyond Python-comment syntax. Architect review recommends an `EvidenceIndexEntry` abstraction with a file-type registry (Python, markdown, YAML/JSON/plist, Xcode capability/entitlement, external test reports, governance documents) rather than expanding `CodeIndexEntry` and `_IMPLEMENTS_RE` indefinitely.
2. **F-007 (IMPORTANT — raised from MINOR by architect review):** Ship a platform-neutral dependency-extractor *interface* with Python (`tools/import_scanner.py`, `ast.parse`-based) as the first adapter. Without an extractor, `visibility_rule_enforcement` is dormant and the boundary model is partially aspirational.
3. **F-008 (IMPORTANT) — traceability semantics:** Decide whether annotations target REQs or DESs. Three options: extend `granularity_match` to accept indirect coverage (REQ covered if any `satisfies`-linked DES has an annotation); add `granularity: design` Module option; or document REQ-level annotation as the convention and require teams to use it. A parser fix (F-001) does **not** resolve F-008 — these are distinct semantic choices.
4. **F-009 (IMPORTANT) — typed evidence statuses on the RTM:** Replace bare `—` placeholder with `linked / missing / not_applicable / manual_evidence_required / configuration_artifact`. Without this the RTM creates a false sense of completeness — formal-looking document, missing central evidence obligation. Even if every annotation were captured, the exporter still cannot distinguish "no implementation" from "implemented by configuration artefact" from "implemented by external evidence". Architect review framed this as the export-evidence-completeness defect; F-001 fix is necessary but not sufficient.
5. **F-010 (IMPORTANT — added by architect review):** REQ-quality drift to function-shaped form was found in three named REQs (`REQ-assured-decomposition-validators-001`, `REQ-assured-traceability-validators-003`, `REQ-assured-skills-001`). v0.2.0 should rewrite drifters at user-visible-capability level AND add a writing-plans-skill checklist + lint candidate for banned phrasings. Tooling alone cannot enforce REQ quality; human review must remain primary.

**The five MINOR findings:**

6. **F-002 (MINOR):** Consider line-range or section-anchor support in `programs.yaml` `paths`. Document whole-directory-only scoping for v0.1.0.
7. **F-003 (MINOR):** Define `satisfies-by-existence` annotation concept for governance documents (constitutions, policy files). Likely subsumed by F-001's `EvidenceIndexEntry` generalisation.
8. **F-004 (MINOR):** No code change — REQ inventory was corrected during authoring; only documentation update needed.
9. **F-005 (MINOR):** Strengthen REQ-001 of traceability-validators to make severity contract unambiguous, OR split warn-vs-block validators into separate REQs.
10. **F-006 (MINOR):** Consider a "see also" / "related-to" mechanism in the REQ format for overlapping intents.

A focused v0.2.0 EPIC must address all 5 IMPORTANT findings as **distinct workstreams** — they reinforce each other but no single fix closes them. The 5 MINOR findings can be picked up opportunistically.
...
This EPIC retrospective consolidates findings from each phase retro. The 10 findings from Phase F (5 IMPORTANT + 5 MINOR after the 2026-05-01 architect review) seed the v0.2.0 backlog. The two architect reviews are at `reviews/2026-05-01-ios-location-services-architect-review.md` and `reviews/20260501T031618Z_review.md`; the controller's analysis of which review recommendations to incorporate is captured in the PR body and the deviation ledger at `research/phase-de-deviation-ledger.md`.
