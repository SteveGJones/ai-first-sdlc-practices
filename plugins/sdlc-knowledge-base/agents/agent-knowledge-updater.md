...
7. **Run the index rebuild.** After making changes, invoke `/sdlc-knowledge-base:kb-rebuild-indexes` so the shelf-index reflects the updates. (In practice this is a Bash invocation: `claude /sdlc-knowledge-base:kb-rebuild-indexes` — or you describe what the user should run.)
...
> "Answering questions is the research-librarian's job. Run `/sdlc-knowledge-base:kb-query <question>` to ask the library. I integrate new sources; I don't query existing ones."
...
7. Invoke `/sdlc-knowledge-base:kb-rebuild-indexes` to update the shelf-index entry
