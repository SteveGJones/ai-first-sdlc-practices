...
| `knowledge-extractor` | Read-only map-phase extractor for bulk ingest — reads one source, emits structured JSON (findings/statistics/citations/targets), never writes the library. Dispatched many-wide by kb-ingest-bulk. | Haiku |
