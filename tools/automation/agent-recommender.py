...
class AgentRecommender:
    ...
    def recommend(self, analysis: Dict, objectives: str = None) -> Dict[str, List[Dict]]:
        ...
        for agent_name, agent_info in self.agent_registry['languages'].items():
            if self._matches_language(agent_info, analysis):
                priority = 'strongly_recommended' if analysis.get('primary_language') in agent_info.get('languages', []) else 'recommended'
    ...
    def _matches_conditions(self, agent_info: Dict, analysis: Dict) -> bool:
        ...
        for condition in conditions:
            if condition in analysis.get('project_types', []):
                return True
            if condition == 'production' and analysis.get('project_size') in ['large', 'enterprise']:
                return True
    ...
    def _adjust_for_objectives(self, recommendations: Dict, objectives: str, analysis: Dict) -> Dict:
        ...
        if any(word in objectives_lower for word in ['ai', 'ml', 'machine learning', 'artificial']):
            self._add_agent(recommendations, 'ml-architect', 'specialized', 
                          'ML system design based on objectives', 'strongly_recommended')
    ...
    def _adjust_for_scale(self, recommendations: Dict, analysis: Dict) -> Dict:
        ...
        if size in ['large', 'enterprise'] or team_size > 10:
            ...
            self._add_agent(recommendations, 'process-auditor', 'core',
                          'Process compliance for large teams', 'strongly_recommended')
            self._add_agent(recommendations, 'quality-metrics-analyst', 'testing',
                          'Quality tracking for complex projects', 'recommended')
    ...
    def _promote_agent(self, recommendations: Dict, agent_name: str, 
    ...
    def _add_agent(self, recommendations: Dict, agent_name: str, category: str,
                   reason: str, level: str = 'recommended'):
    ...
    def generate_phase_recommendations(self, phase: str, analysis: Dict) -> List[Dict]:
        ...
        phase_agents = {
            'planning': ['solution-architect', 'security-architect', 'database-architect'],
            'development': ['language-experts', 'framework-specialists', 'api-designer'],
            'testing': ['test-manager', 'test-strategist', 'performance-tester'],
            'deployment': ['kubernetes-architect', 'ci-cd-architect', 'cloud-architects'],
            'maintenance': ['process-auditor', 'quality-metrics-analyst', 'security-reviewer']
        }
        ...
        if phase in phase_agents:
            for agent_type in phase_agents[phase]:
                if agent_type == 'language-experts':
                    ...
                    recommendations.append({
                        'name': agent_type,
                        'reason': f"Critical for {phase} phase"
                    })
...
@click.command()
@click.option('--analysis-file', type=click.Path(exists=True),
...
@click.option('--phase', type=click.Choice(['planning', 'development', 'testing', 
