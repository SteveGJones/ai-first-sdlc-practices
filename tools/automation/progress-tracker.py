...
class TodoItem:
    ...
    def __init__(self, content: str, status: str = "pending", 
                 priority: str = "medium", id: Optional[str] = None,
...
class ProgressTracker:
    ...
    def list_todos(self, status: Optional[str] = None, 
                   branch: Optional[str] = None) -> List[TodoItem]:
        ...
        if not todos:
            ...
            for todo in todos:
                status_icon = "✅" if todo.status == TodoStatus.COMPLETED else \
                             "🚧" if todo.status == TodoStatus.IN_PROGRESS else \
                             "🚫" if todo.status == TodoStatus.BLOCKED else "⏸️"
                
                priority_icon = "🔴" if todo.priority == TodoPriority.CRITICAL else \
                               "🟡" if todo.priority == TodoPriority.HIGH else \
                               "🟢" if todo.priority == TodoPriority.MEDIUM else "⚪"
