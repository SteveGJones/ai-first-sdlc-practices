...
def extract_agent_metadata(file_path: Path) -> Dict[str, Any]:
    ...
    capabilities = []
    cap_match = re.search(r'(?:what i do|capabilities|responsibilities|expertise).*?\n(.*?)(?:\n#|\n\n#|$)', 
...
def build_catalog():
    ...
    agent_files = [f for f in agent_files if 'README' not in f.name and 'template' not in f.stem.lower()]
    ...
    print(f"Successfully created AGENT-CATALOG.json with {catalog['total_agents']} agents")
...
def create_readable_index(catalog: Dict[str, Any]):
    ...
    for category in sorted(by_category.keys()):
        ...
        output.append(f"### {category.replace('-', ' ').title()} ({len(agents)} agents)\n\n")
