...
if [ -z "$STAGED_PYTHON_FILES" ]; then
    echo "✅ No Python files staged, skipping quality gate"
    exit 0
fi
...
echo "
🔍 Running CodeQL-style analysis..."
if ! python tools/validation/pre-push-quality.py --codeql; then
    echo "❌ Static analysis found issues. Fix these before committing."
    echo "💡 Tip: Run the full analysis with --verbose for detailed suggestions"
    exit 1
fi
...
if ! python tools/validation/local-validation.py --quick; then
    echo "❌ Quick validation failed. Review issues above."
    exit 1
fi
...
echo "🚀 Running Enhanced Pre-Push Quality Gate..."
...
🔧 Fix suggestions:"
    ...
    echo "
💡 These checks mirror CI/CD validation - fixing them locally saves time!"
    exit 1
fi
...
if [ ${#FIRST_LINE} -gt 72 ]; then
    echo "❌ First line too long (maximum 72 characters, got ${#FIRST_LINE})"
    exit 1
fi
...
def main():
    parser = argparse.ArgumentParser(description='Install enhanced git hooks for quality gate')
    ...
    print("🔧 Installing Enhanced Git Hooks for AI-First SDLC...")
    ...
    if existing_hooks and not args.force:
        ...
        for hook_name in existing_hooks:
            ...
            print(f"📦 Backed up {hook_name} to {hook_name}.backup")
    ...
    print(f"\n{'='*50}")
    print(f"🎯 Enhanced Git Hooks Installation Summary")
    print(f"{'='*50}")
    
    if success_count == 3:
        print("✅ All hooks installed successfully!")
        print("\n📋 Installed hooks:")
        print("  • pre-commit: Syntax + CodeQL-style analysis")
        print("  • pre-push: Comprehensive quality gate")
        print("  • commit-msg: Conventional commit format")
        print("\n💡 Benefits:")
        print("  ✓ Catches CodeQL issues before CI/CD")
        print("  ✓ Enforces consistent code quality")
        print("  ✓ Prevents push-fail-fix cycles")
        print("  ✓ Validates commit message format")
        print("\n🚀 Ready! Your next commit will run enhanced validation.")
        return 0
    else:
        print(f"⚠️  Only {success_count}/3 hooks installed successfully")
