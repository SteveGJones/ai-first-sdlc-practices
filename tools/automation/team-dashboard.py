...
class TeamDashboard:
    ...
    def _get_readiness_score(self, formation_type: str) -> float:
        ...
        try:
            ...
            result = subprocess.run([
                "python", "tools/automation/validate-team-readiness.py",
    ...
    def _get_recent_activity(self) -> List[str]:
        ...
        if log_files:
            recent_logs = [f for f in log_files 
                          if (datetime.now() - datetime.fromtimestamp(f.stat().st_mtime)).hours < 24]
            if recent_logs:
        ...
        if proposal_dir.exists():
            recent_proposals = [f for f in proposal_dir.glob("*.md")
    ...
    def display_dashboard(self, metrics: TeamMetrics, clear_screen: bool = True):
        ...
        # Header
        formation_info = self.dashboard_config["formations"].get(metrics.formation_type, 
                                                               {"symbol": "🤖", "color": "white"})
    ...
    def _generate_alerts(self, metrics: TeamMetrics) -> List[str]:
        ...
        if metrics.agent_count < config["min_agents"]:
            alerts.append(f"⚠️ WARNING: Only {metrics.agent_count} agents active, {config['min_agents']} minimum for {metrics.formation_type}")
    ...
    def _run_chemistry_exercise(self, formation_type: str):
        ...
        try:
            result = subprocess.run([
                "python", "tools/automation/team-chemistry.py",
                "--exercise", "simple-feature-challenge",
                "--formation", formation_type
            ], timeout=120)
    ...
    def _show_detailed_status(self, metrics: TeamMetrics):
        ...
        if len(self.metrics_history) > 1:
            ...
            for i, m in enumerate(self.metrics_history[-5:]):
                click.echo(f"  {i+1}. {m.last_updated.strftime('%H:%M')} - "
...
@click.option('--formation', type=click.Choice(['builder', 'specialist', 'innovator', 'transformer', 'orchestrator']),
              default='builder', help='Formation type to monitor')
