...
import importlib.util
spec = importlib.util.spec_from_file_location("validate_pipeline", 
                                              script_dir / "validate-pipeline.py")
...
class ProgressiveValidationPipeline(ValidationPipeline):
    ...
    LEVEL_CHECKS = {
        ...
        'production': {
            'required': ['branch', 'proposal', 'architecture', 'technical-debt', 
        ...
        'enterprise': {
            'required': ['branch', 'proposal', 'architecture', 'technical-debt',
                        'type-safety', 'security', 'tests', 'retrospective',
                        'code-quality', 'dependencies', 'commit-history', 'logging'],
            'optional': [],
            'skip': []
        }
    }
    ...
    def _determine_current_phase(self) -> str:
        ...
        if arch_dir.exists() and any(arch_dir.iterdir()):
            recent_arch = any(f.stat().st_mtime > (datetime.now().timestamp() - 3600) 
    ...
    def check_technical_debt(self) -> Dict:
        ...
        if self.level == 'prototype':
            ...
            try:
                cmd = ['grep', '-r', '-n', '-E', 'TODO|FIXME|HACK', 
                       '--include=*.py', '--include=*.js', '--include=*.ts', '.']
                output = subprocess.run(cmd, cwd=self.project_root, 
                                     capture_output=True, text=True)
                
                if output.returncode == 0:
                    todo_count = len(output.stdout.strip().split('\n'))
                    result['details'] = f'Found {todo_count} TODO/FIXME markers (allowed in prototype)'
                    result['status'] = 'info'
...
def main():
    ...
    pipeline.level_config = pipeline.LEVEL_CHECKS.get(
        pipeline.level, 
        pipeline.LEVEL_CHECKS['production']
    )
