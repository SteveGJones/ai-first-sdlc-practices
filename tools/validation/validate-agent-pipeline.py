    ...
    def validate(self, file_path: Path) -> List[PipelineResult]:
        ...
        for section in self.REQUIRED_SECTIONS:
            if re.search(
                ...
                results.append(
                    PipelineResult(
                        "structure",
                        f"Required section found: '{section}'",
                        severity="success",
                    )
                )
            else:
                results.append(
                    PipelineResult(
                        "structure", f"Missing required section: '{section}'"
                    )
                )
        ...
        if area_count < self.MIN_RESEARCH_AREAS:
            results.append(
                PipelineResult(
                    "research-areas",
                    f"Too few research areas: {area_count} (minimum {self.MIN_RESEARCH_AREAS})",
                )
            )
            ...
            results.append(
                PipelineResult(
                    "research-areas",
                    f"Too many research areas: {area_count} (maximum {self.MAX_RESEARCH_AREAS})",
                    severity="warning",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "research-areas",
                    f"Research areas count: {area_count}",
                    severity="success",
                )
            )
        ...
        if len(question_lines) < 5:
            results.append(
                PipelineResult(
                    "questions",
                    f"Few research questions: {len(question_lines)} (recommend 15+)",
                    severity="warning",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "questions",
                    f"Research questions found: {len(question_lines)}",
                    severity="success",
                )
            )
        ...
        if len(content) < 500:
            results.append(
                PipelineResult(
                    "content", "Research prompt too short (minimum 500 characters)"
                )
            )

        bracket_placeholders = len(
            re.findall(r"\[(?:AGENT|ROLE|DOMAIN|CUSTOMIZE)[^\]]*\]", content)
        )
        if bracket_placeholders > 0:
            results.append(
                PipelineResult(
                    "placeholders",
                    f"Found {bracket_placeholders} unfilled placeholder(s)",
                    severity="warning",
                )
            )
...
class ReferenceAgentValidator:
    ...
    def validate(self, file_path: Path) -> List[PipelineResult]:
        ...
        for tag in ["ARCHETYPE:", "BASED ON:", "USE WHEN:"]:
            if tag not in content:
                results.append(
                    PipelineResult(
                        "header",
                        f"Missing {tag.rstrip(':')} comment in header",
                        severity="warning",
                    )
                )
        ...
        if re.search(r"^---\s*\n(.*?)\n---", content, re.MULTILINE | re.DOTALL):
            results.append(
                PipelineResult(
                    "frontmatter", "YAML frontmatter present", severity="success"
                )
            )
        else:
        ...
        if customize_count < 3:
            results.append(
                PipelineResult(
                    "placeholders",
                    f"Only {customize_count} [CUSTOMIZE] placeholder(s)",
                    severity="warning",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "placeholders",
                    f"Found {customize_count} [CUSTOMIZE] placeholders",
                    severity="success",
                )
            )
        ...
        if line_count > 120:
            results.append(
                PipelineResult(
                    "size",
                    f"Reference agent is {line_count} lines (target: 50-80)",
                    severity="warning",
                )
            )
...
class ProductionAgentValidator:
    ...
    def validate(
        self, file_path: Path, require_research: bool = False
    ) -> List[PipelineResult]:
        results = []
        if not file_path.exists():
            results.append(PipelineResult("file", f"File not found: {file_path}"))
        ...
        research_prompt = (
            ...
            / "agent_prompts"
            / f"research-{agent_name}.md"
        )
        if research_prompt.exists():
            results.append(
                PipelineResult(
                    "research",
                    f"Research prompt found: {research_prompt.name}",
                    severity="success",
                )
            )
            ...
            results.append(
                PipelineResult(
                    "research",
                    f"No research prompt found for '{agent_name}'."
                    f" Production agents MUST have a research prompt"
                    f" at agent_prompts/research-{agent_name}.md",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "research",
                    f"No research prompt found for '{agent_name}'."
                    f" Consider creating agent_prompts/research-{agent_name}.md"
                    f" for research-grounded content",
                    severity="warning",
                )
            )
        ...
        if remaining:
            results.append(
                PipelineResult(
                    "placeholders",
                    f"Found {len(remaining)} unfilled [CUSTOMIZE] placeholder(s) - agent is incomplete",
                )
            )
        else:
            results.append(
            ...
            for field in ["name:", "description:", "examples:", "color:"]:
                if field in fm_content:
                    results.append(
                        PipelineResult(
                            "frontmatter",
                            f"Required field '{field.rstrip(':')}' present",
                            severity="success",
                        )
                    )
                else:
                    results.append(
                        PipelineResult(
                            "frontmatter",
                            f"Missing required field: '{field.rstrip(':')}'",
                        )
                    )
        else:
            results.append(PipelineResult("frontmatter", "Missing YAML frontmatter"))
        ...
        if re.search(r"You are .+", body):
            results.append(
                PipelineResult("content", "Role statement present", severity="success")
            )
        else:
            results.append(
                PipelineResult(
                    "content",
                    "Missing role statement ('You are...')",
                    severity="warning",
                )
            )
        ...
        if (
            "core competencies include:" in body.lower()
            or "core competencies:" in body.lower()
        ):
            results.append(
                PipelineResult(
                    "content", "Core competencies section present", severity="success"
                )
            )
        else:
            results.append(
                PipelineResult(
                    "content", "Missing core competencies section", severity="warning"
                )
            )
        ...
        if body_words < self.MIN_DOMAIN_WORDS:
            results.append(
                PipelineResult(
                    "depth",
                    f"Agent content is thin: {body_words} words (minimum {self.MIN_DOMAIN_WORDS})",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "depth", f"Agent content: {body_words} words", severity="success"
                )
            )
        ...
        if re.search(r"##\s*(Boundaries|Collaboration|Handoff)", body, re.IGNORECASE):
            results.append(
                PipelineResult(
                    "integration",
                    "Boundaries/collaboration section present",
                    severity="success",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "integration",
                    "Missing boundaries section - define handoff points with other agents",
                    severity="warning",
                )
            )
...
class InfrastructureValidator:
    ...
    def validate(self, base_dir: Path) -> List[PipelineResult]:
        ...
        checks = [
            (
                base_dir / "templates" / "agent-research-prompt.md",
                "Research prompt template",
                True,
            ),
            (
                base_dir / "docs" / "RESEARCH-PROMPT-GUIDE.md",
        ...
        for path, label, required in checks:
            if path.exists():
                results.append(
                    PipelineResult(
                        "infrastructure", f"{label} exists", severity="success"
                    )
                )
            else:
                ...
                results.append(
                    PipelineResult(
                        "infrastructure", f"Missing {label}: {path}", severity=sev
                    )
                )
        ...
        if ref_dir.exists():
            ...
            results.append(
                PipelineResult(
                    "infrastructure",
                    f"Reference agents directory: {len(ref_agents)} archetype(s)",
                    severity="success",
                )
            )
            if len(ref_agents) < 3:
                results.append(
                    PipelineResult(
                        "infrastructure",
                        f"Few reference agents: {len(ref_agents)} (recommend 5)",
                        severity="warning",
                    )
                )
        else:
            results.append(
                PipelineResult(
                    "infrastructure", f"Missing reference agents directory: {ref_dir}"
                )
            )
        ...
        if prompts_dir.exists():
            results.append(
                PipelineResult(
                    "infrastructure",
                    "Agent prompts directory exists",
                    severity="success",
                )
            )
        else:
            results.append(
                PipelineResult(
                    "infrastructure",
                    "Missing agent prompts directory",
                    severity="warning",
                )
            )
        ...
        if guide.exists() and "Agent Creation Pipeline" in guide.read_text():
            results.append(
                PipelineResult(
                    "documentation",
                    "AGENT-CREATION-GUIDE.md includes pipeline overview",
                    severity="success",
                )
            )
        elif guide.exists():
            results.append(
                PipelineResult(
                    "documentation",
                    "AGENT-CREATION-GUIDE.md missing pipeline section",
                    severity="warning",
                )
            )
        else:
            results.append(
                PipelineResult("documentation", "Missing AGENT-CREATION-GUIDE.md")
            )
...
def output_results(results: List[PipelineResult], command: str, output_json: bool):
    ...
    if output_json:
        output = {
            ...
            "results": [
                {"check": r.check, "message": r.message, "severity": r.severity}
                for r in results
            ],
        }
...
@click.option(
    "--require-research",
    is_flag=True,
    help="Fail if no corresponding research prompt exists",
)
def validate_production_agent(file: str, output_json: bool, require_research: bool):
    ...
    results = ProductionAgentValidator().validate(
        Path(file), require_research=require_research
    )
