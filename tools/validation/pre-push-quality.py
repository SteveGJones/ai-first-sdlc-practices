...
class CodeQLStyleAnalyzer:
    ...
    def analyze_undefined_methods(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            undefined_patterns = [
                (r'\.setup\(.*\)', 'Method "setup" may not exist - check if it should be "configure"'),
                (r'\.contains\(', 'Method "contains" does not exist on strings - use "in" operator'),
                (r'urlopen\(', 'urllib.urlopen is deprecated - use urllib.request.urlopen'),
            ]
