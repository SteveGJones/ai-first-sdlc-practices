...
CORE_CATEGORIES = {
    'core/governance', 'core/architecture', 'core/testing', 'core/review'
}
...
class AgentValidator:
        ...
        try:
            ...
            if 'expertise' in metadata:
                if not isinstance(metadata['expertise'], list):
                    self.errors.append((
                        agent_name,
                        "Expertise must be a list"
                    ))
                elif len(metadata['expertise']) < 3:
                    self.warnings.append((
                        agent_name,
                        "Agent should have at least 3 areas of expertise"
                    ))
            ...
            if 'triggers' in metadata:
                if not isinstance(metadata['triggers'], list):
                    ...
                    self.warnings.append((
                        agent_name,
                        "Agent should have at least 2 triggers"
                    ))
    ...
    def _is_valid_category(self, category: str) -> bool:
        ...
        valid_patterns = [
            'core/', 'languages/', 'ux/', 'devops/', 
            'data/', 'compliance/', 'ai-ml/', 'agent-development/',
            'mcp/', 'testing/', 'architecture/', 'review/'
        ]
            ...
            if 'dependencies' in metadata:
                for dep in metadata['dependencies']:
                    if dep not in self.all_agents:
                        ...
                        if not any(dep.startswith(cat) for cat in [
                            'testing/', 'review/', 'architecture/', 'languages/'
                        ]):
                            self.warnings.append((
                                agent_name,
