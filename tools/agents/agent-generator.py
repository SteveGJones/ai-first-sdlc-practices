...
class AgentValidator:
    ...
    def validate_frontmatter(self, frontmatter_str: str) -> Dict:
        ...
        for i, example in enumerate(data["examples"]):
            ...
            if missing_example:
                raise ValueError(
                    f"Example {i+1} missing fields: {', '.join(missing_example)}"
                )
    ...
    def validate_file(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            return {"valid": False, "path": str(filepath), "error": str(e)}
...
@click.argument("name")
@click.option(
    "--output", "-o", type=click.Path(path_type=Path), help="Output path for template"
)
...
def extract(agent_file: Path, output: Optional[Path]):
    ...
    try:
        ...
        click.echo(
            f"  Edit this file and regenerate with: python {__file__} generate {output}"
        )
