...
- Verify the project has a knowledge base configured (the `[Knowledge Base]` section in `CLAUDE.md` exists). If not, run `/sdlc-core:kb-init` first.
...
- It does not query the library — that's `/sdlc-core:kb-query`
- It does not lint the library — that's `/sdlc-core:kb-lint`
- It does not validate citations — that's `/sdlc-core:kb-validate-citations`
...
```
/sdlc-core:kb-ingest library/raw/forsgren-2024-cycle-time-update.md
```

**Ingest from a URL:**
```
/sdlc-core:kb-ingest https://cloud.google.com/devops/state-of-devops/2024
```

**Ingest pasted text (e.g., a conversation excerpt with research findings):**
```
/sdlc-core:kb-ingest "<paste the excerpt here>"
```
