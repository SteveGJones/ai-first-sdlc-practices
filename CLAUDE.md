...
- **EPIC #211** — kb-offline: optional LangGraph+Ollama offline backend for `sdlc-knowledge-base` (off by default). Branch `feature/211-kb-offline-langgraph` (one accumulating branch, no PR until deliberate). M0–M2 complete; M3 (embeddings accelerator) in progress — M3a/M3b/M3c-1 done; **M3c-2 (accelerated federated query) COMPLETE** (all 9 tasks committed + two-stage reviewed, whole-slice review READY, live smoke ran end-to-end on `gemma4:12b`; branch kept). **M3c-3 (shareable-fingerprint "find who to ask" discovery) DESIGNED + PLANNED, implementation not started** — spec `docs/superpowers/specs/2026-06-16-kb-offline-M3c3-shareable-fingerprint-discovery-design.md`, plan `docs/superpowers/plans/2026-06-16-kb-offline-M3c3-shareable-fingerprint-discovery.md` (7 tasks, subagent-driven). M3c-3 remaining after: M3d (semantic conflict lint). **Local generation model is `gemma4:12b`** (switched from `gpt-oss:20b`, which swapped this machine; embeddings stay `nomic-embed-text`). **gemma4:12b re-ratification was running when this session ended — verify the report exists / re-run if not** (it emits a trailing `<|tool_response>` token after JSON that fails synthesize validation — a model-output-contamination finding). Unit tests use `FakeBackend` (no Ollama); pin `num_ctx=8192` for any live Ollama run to avoid swap. Restart prompt: `tmp/restart-prompt-211-m3c3.md`.
