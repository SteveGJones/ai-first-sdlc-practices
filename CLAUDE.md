...
- **#237 — poker-capstone (`sdlc-model-council` research)**. Branch: `feature/council-poker-capstone`. Sonnet baseline (P1-P10) done and clean; Haiku's run done, deliberately stopped at P6 (fail-fast, 2 genuine FAILs). Next model for the P11 roster is a local MLX model, `Qwen2.5-Coder-14B-Instruct-4bit`. **Two blockers cleared 2026-07-31**: the `mlx_lm.server` prompt-cache OOM is fixed (`--prompt-cache-size 2`; note `--max-concurrent` does NOT exist in mlx-lm 0.31.3 — use `--decode-concurrency`/`--prompt-concurrency`), and OpenCode (Path A) is **rejected for code-reproduction work** — its quote-escaping corruption is now reproduced on 14B, not just 7B (1 corrupted bug-fix item; 3 code-review items on the same model were clean, so the fault is task-shaped: reproduced source, not JSON/prose). Because the `mlx:` adapter is text-only, an **agentic wrapper** was built and verified (`research/poker-capstone/harness/{file_blocks,mlx_client,local_agent,run_local}.py`) so P5/P6-style build phases are reachable. **Next: run 14B through the ladder starting at P1, in order, fail-fast** via `python -m harness.run_local`. Note **P1-P10 are the phases; P11 is the cross-model roster exercise, not a phase to jump to** — 0 of 11 poker phases have ever been run through OpenCode. See `RESTART-PROMPT.md` and memory `poker-capstone-p11-status`.
...
| `sdlc-model-council` | Cross-model council — assess the models reachable via locally installed peer agentic CLIs (codex, agy, opencode; extensible via one-directory adapters) against a standardized problem stack, cast them into a roster, and run cross-model fan-out plays (v1: Diff+Synthesis) chosen per project. Built on a unified delegation contract with graded permission postures. In-session and uncontainerised — no Archon, no Docker, no DAG, unlike `sdlc-workflows` (2 agents: delegation-runner, council-judge) |
