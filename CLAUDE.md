...
- **EPIC #96** — Containerised Claude Code workers. **PR #159 open, pending merge.** 139 commits on `feature/96-sdlc-workflows`. Delivers: sdlc-workflows plugin with 6 skills, 4 bundled workflows, three-tier Docker model, three-tier credential resolver, three-tier termination (budget→timeout→hard kill), image-tag allowlist, SQLite schema probe, bash-node preprocessing, and full documentation. 280 unit tests + 22 container integration tests pass. Dogfood verified 7/7 on sdlc-parallel-review. Upstream Archon issues filed: #1125, #1126, #1321, #1322, #1323. CI/CD automation deferred to separate issue.
