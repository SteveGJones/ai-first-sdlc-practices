...
- **EPIC #211** — kb-offline: optional LangGraph+Ollama offline backend for `sdlc-knowledge-base` (off by default). Branch `feature/211-kb-offline-langgraph` (one accumulating branch, no PR until deliberate). M0–M2 complete; M3 (embeddings accelerator) in progress — M3a/M3b/M3c-1 done, **M3c-2 (accelerated federated query)** mid-execution (subagent-driven: Tasks 1–7 of 9 committed; Task 8 CLI routing + Task 9 live smoke remaining). Plan `docs/superpowers/plans/2026-06-13-kb-offline-M3c2-accelerated-federation.md`. **Local generation model is `gemma4:12b`** (switched from `gpt-oss:20b`, which drove this machine into swap; embeddings stay `nomic-embed-text`). Eval/ratification re-run pending against `gemma4:12b`. Unit tests use `FakeBackend` (no Ollama); pin `num_ctx=8192` for any live Ollama run to avoid swap.
