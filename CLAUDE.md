...
- **EPIC #211** — kb-offline: optional LangGraph+Ollama offline backend for `sdlc-knowledge-base` (off by default). Branch `feature/211-kb-offline-langgraph` (one accumulating branch, no PR until deliberate). M0–M2 complete; M3 (embeddings accelerator) in progress — M3a/M3b/M3c-1 done; **M3c-2 (accelerated federated query) COMPLETE** (all 9 tasks committed + two-stage reviewed, whole-slice review READY, live smoke ran end-to-end on `gemma4:12b`; branch kept). **M3c-3 (shareable-fingerprint discovery) DESIGNED + PLANNED (spec/plan `2026-06-16-*`, not built); M3d (semantic conflict lint) remaining. **Select/verifier-hardening track: SLICE 1 (abstention contract) COMPLETE & RATIFIED 2026-06-22** — 15-task plan subagent-driven, 14 commits `e8c3039..735f59c` + ratification `6d3cdb1`, 317 non-live tests, whole-slice ✅; spec `docs/superpowers/specs/2026-06-19-kb-offline-abstention-contract-design.md`, plan `docs/superpowers/plans/2026-06-19-kb-offline-abstention-contract.md`. Live gemma4:12b ratification moved abstention_recall 0.818→1.0, fact_recall 0.787→0.867 (PASS), routing_precision 0.795→0.935 (PASS); **still FAIL on 2 residual margins** (abstention_precision 0.917 — 2 false abstentions inc. a synthesize page-id hallucination → Slice 2 fuzzy-id target; first_pass_json 0.926 — trailing `<|tool_response>` sentinel, pipeline handles it, metric measures raw pre-sanitize → Slice 3 scorer target). Default STAYS Anthropic. Next: Slice 2 (fuzzy id-normalize), Slice 3 (scorer evolution + gate wiring). **Local model `gemma4:12b`** (gpt-oss:20b swaps machine; embeddings `nomic-embed-text`). Unit tests use `FakeBackend`; pin `num_ctx=8192` for live Ollama. See memory `project-kb-offline-select-hardening`.
