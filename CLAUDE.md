...
- **EPIC #96** — Containerised Claude Code workers. Phases 1-5 complete. Hardened for local workstation use: capability dropping, signal handling, configurable timeout, Docker healthcheck, credential staging. Full documentation suite (CLAUDE-CONTEXT reference, quickstart, author-workflow skill, troubleshooting). Branch `feature/96-sdlc-workflows`. CI/CD automation is a separate future issue.
